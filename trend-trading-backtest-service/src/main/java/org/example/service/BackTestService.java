...
public class BackTestService {
    @Qualifier("org.example.client.IndexDataClient")
    ...
    public Map<String,Object> simulate(int ma, float sellRate, float buyRate, float serviceCharge, List<IndexData> indexDatas)  {
        ...
        float share = 0;//股份，按股价可以买入多少股
        float value = 0;//目前手上持有的现金或者股票的价值

        int winCount = 0; //盈利次数
        float totalWinRate = 0;//总盈利率
        float avgWinRate = 0;//平均盈利率
        ...
        for (int i = 0; i<indexDatas.size() ; i++) {
            ...
            float rate = value/initCash; //这里比率的计算需要思考一下为什么这么设置。
